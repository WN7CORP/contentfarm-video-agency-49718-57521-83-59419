...
export const FlashcardViewer = ({
  ...
  return <div className="w-full max-w-2xl mx-auto p-4 space-y-4">
      <div className="text-center text-sm text-muted-foreground mb-2">
        Flashcard {currentIndex + 1} de {flashcards.length}
      </div>

      <ReactCardFlip isFlipped={isFlipped} flipDirection="horizontal">
        <div onClick={() => setIsFlipped(true)} className="min-h-[300px] bg-card border-2 border-primary rounded-xl p-8 flex flex-col cursor-pointer hover:shadow-lg transition-shadow relative">
          {tema && (
            <p className="text-xs text-primary/60 mb-3 absolute top-4 left-4">
              {tema}
            </p>
          )}
        ...
        <div onClick={() => setIsFlipped(false)} className="min-h-[350px] bg-card border-2 border-primary rounded-xl p-8 cursor-pointer hover:shadow-lg transition-shadow relative">
          {tema && (
            <p className="text-xs text-primary/60 mb-3 absolute top-4 left-4">
              {tema}
            </p>
          ...
          <div className="space-y-4 mt-6">
            ...
            {currentCard.exemplo && <div className="bg-background/50 rounded-lg p-4 border border-primary/20">
