...
const getPreviousPageTitle = (pathname: string): string => {
  ...
  if (pathname.startsWith("/videoaulas/")) return "Videoaulas";
  if (pathname.startsWith("/flashcards/")) return "Flashcards";
  if (pathname === "/cursos" || pathname === "/videoaulas" || pathname === "/flashcards" || pathname === "/resumos-juridicos" || pathname === "/plano-estudos" || pathname === "/professora") return "Aprender";
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
        <div className="flex h-16 md:h-14 items-center justify-between px-4 md:px-6 max-w-7xl mx-auto">
          <div className="flex items-center gap-3 md:gap-2">
            ...
            />
            <h1 className="text-xl md:text-lg font-bold text-foreground font-cinzel tracking-wide">
              Direito Premium
            </h1>
          </div>
        </div>
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
      <div className="flex h-16 md:h-14 items-center px-4 md:px-6 gap-3 max-w-7xl mx-auto">
        <button 
          onClick={() => navigate(-1)} 
          className="flex items-center gap-2 md:gap-2 px-3 md:px-3 py-2.5 md:py-2 hover:bg-secondary rounded-lg transition-all hover:scale-105 border border-border/50 hover:border-border flex-shrink-0 group"
